...
sed -e 's/\(^[ ]*(func $printDouble\)/\    \;; (func $printDouble/g' -e 's/\(^[ ]*(func $printString\)/\    \;; (func $printString/g' -e 's/\(^[ ]*(func $printError\)/\    \;; (func $printError/g' -e 's/\(^[ ]*;; (import\)/\    \(import/g' -e 's/\(^[ ]*;; (import\)/\    \(import/g' -e 's/\(^[ ]*(mem\)/\    \;; (mem/g' src/get_mem.wat > $filepath-temp.wat
